...
namespace WellFired.Guacamole.Unit.Bindable
{
	...
	public class IntBindableObjectTests
	{
		...
		public void OneWayBindingInverseTest()
		{
			...
			source.Bind(BindableTestObject.IntProperty, nameof(ContextObject.Value), BindingMode.OneWay);
		...
		public void ReadOnlyBindingTest()
		{
			...
			var bindingContext = new ContextObject();
			source.BindingContext = bindingContext;
			source.Bind(BindableTestObject.IntProperty, nameof(ContextObject.Value), BindingMode.ReadOnly);
			...
			source.Value = 15;
			Assert.That(source.Value, Is.EqualTo(10));
			Assert.That(source.Value, Is.EqualTo(bindingContext.Value));
		}
