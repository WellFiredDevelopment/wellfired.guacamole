...
namespace WellFired.Guacamole.Unit.ImageSource
{
    ...
    public class Given_AFileSourceHandler
    {
        ...
        public void When_ThePathToTheStreamIsValid_TheStreamIsReturned()
        {
            ...
            mockedFileSystem.GetStream(Arg.Any<string>(), Arg.Any<FileMode>()).Returns(MockedData);
        ...
        public void TestingAsyncBehaviour()
        {
            ...
            var mockedFileSystem = Substitute.For<IFileSystem>();
            mockedFileSystem.GetStream(Arg.Any<string>(), Arg.Any<FileMode>()).Returns(ReturnMockedDataAfterLongTime);
            ...
            Assert.That(() =>
            {
                AsyncHelpers.RunSync(() =>
                {
                    return TaskEx.WhenAll(
                        TaskEx.Run(() => fileSourceHandler.Handle(cancellationTokenSource.Token), cancellationTokenSource.Token),
